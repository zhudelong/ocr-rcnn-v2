...
chars_coding = {1: '1',  2: '2',  3: '<',  4: '>',  5: '3', 6: '!',
                7: '4',  8: '*',  9: '5',  10: '?', 11: '6', 12: '7',
                13: '8', 14: '%', 15: '0', 16: '$', 17: '9', 18: 'G',
                19: 'B', 20: '(', 21: ')', 22: '^', 23: 'L', 24: 's',
                25: 'P', 26: '-', 27: 'M', 28: '#', 29: 'U', 30: 'D',
...
def draw_bounding_box_on_image(image,
  ...
  # display localization rectangles
  draw.line([(left, top), (left, bottom), (right, bottom),
             (right, top), (left, top)], width=thickness, fill=color)

  # display button detection and localization result
  try:
  ...
  for display_str in display_str_list[::-1]:
    detection_char = display_str.split('_')[0]
    recognition_char = display_str.split('_')[-1]
    recog_width, recog_height = font.getsize(recognition_char)
    text_width, text_height = font.getsize(detection_char)
    ...
    draw.text(
        ...
        detection_char, fill='black', font=font)
    x_center = (left + right) / 2.0
    y_center = (top + bottom) / 2.0
    draw.text((x_center-recog_width/2.0, y_center-recog_height/2.0),
              recognition_char, fill=color, font=font_recog)
        ...
        if not agnostic_mode:
          ...
          if predict_chars is not None:
              ...
              if predict_chars[i][2] != 46:
                  str_disp += char_3
          display_str = '{}: {}%_{}'.format(
              class_name, int(100*scores[i]), str_disp)
        else:
